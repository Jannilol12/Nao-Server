...
/**
 * Wrote gandamstyle at the beginning of the Seminar with the virtual robot, but when we tested it on the Nao itself, it didn't worked very well.
 * So we didn't worked on the class anymore....if you want try to let im dance the Gangnam style
 */

public class gangnam implements SendClassName {
    /**
     * Starting the dance
     * @param args here args will be not used...
     */
    @Override
        ...
        try{
            ...
            while(count < 16) {
                if(up) {
                    ...
                    p.setAngles(motors.RShoulderPitch.name, Math.toRadians(0), speed);
                }else {
                    p.setAngles(motors.LShoulderPitch.name, Math.toRadians(50), speed);
                    p.setAngles(motors.RShoulderPitch.name, Math.toRadians(40), speed);
                }
            ...
            p.setAngles(motors.RShoulderRoll.name, Math.toRadians(6), speed);
            p.setAngles(motors.RElbowYaw.name, Math.toRadians(0), speed);
    ...
    /**
     *
     * @return the name of the class for the list
     */
    ...
    /**
     *
     * @return null, because the user should only start the class and the class need no input, like a String
     */
	@Override
