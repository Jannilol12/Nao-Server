...
public class events {
	...
	public static void startFootContactChanged() {
		...
		try {
			footContact = currentApplication.getAlMemory().subscribeToEvent("footContactChanged",
...
//		}
	...
	public static void startSpeechRecognition() {
		try {
			...
			speechRecognition = currentApplication.getAlMemory().subscribeToEvent("WordRecognized",
					new EventCallback<ArrayList>() {
						@Override
						public void onEvent(ArrayList o) throws InterruptedException, CallError {
							System.out.println("SubscribeToEevent Raised:");
							System.out.println(o.toString());
							System.out.println(currentApplication.getAlMemory().getData("WordRecognized") + ""
	...
	public static void startSonar() {
		...
		// The results of the first echo detected on each receiver are in Value, the 9
		// following echoes are from Value1 to Value9.
		//
		// Value of 0 means an error. A value of Max Detection range means no echo.
		//
		// For example, if Value contains 0,40, Value1 1,2 and Value2 Max Detection
		// range, the following values (3 to 9) will contain Max Detection range too.
		// It probably means you have the echo of the ground at 0,40m and another object
		// at 1,2m. Left and Right sensors work the same way and allow you to locate
		// objects.
		try {
			currentApplication.getAlSonar().subscribe("ALSonar");

			String distanceLeft = currentApplication.getAlMemory().getData("Device/SubDeviceList/US/Left/Sensor/Value")
					.toString();
			String distanceRight = currentApplication.getAlMemory()
					.getData("Device/SubDeviceList/US/Right/Sensor/Value").toString();
			...
			sonar = currentApplication.getAlMemory().subscribeToEvent("SonarLeftDetected", new EventCallback<Float>() {
				...
				public void onEvent(Float o) throws InterruptedException, CallError {
					...
					String distanceLeft = currentApplication.getAlMemory()
							.getData("Device/SubDeviceList/US/Left/Sensor/Value").toString();
					String distanceRight = currentApplication.getAlMemory()
							.getData("Device/SubDeviceList/US/Right/Sensor/Value").toString();
					System.out.println("Distance Left: " + distanceLeft + "|" + "Distance Right: " + distanceRight);
					System.out.println("---------------------------------");
	...
	public static void startFaceDectection() {
		try {
			...
			faceDetection = currentApplication.getAlMemory().subscribeToEvent("FaceDetected",
					new EventCallback<ArrayList>() {
						@Override
						public void onEvent(ArrayList o) throws InterruptedException, CallError {
							System.out.println("SubscribeToEevent Raised: ");
	...
	public static void startBarcodeReader() {
		try {
			...
			System.out.println("---------------------------");
			barcodeReader = currentApplication.getAlMemory().subscribeToEvent("BarcodeReader/BarcodeDetected",
					new EventCallback<ArrayList>() {
						@Override
						public void onEvent(ArrayList o) throws InterruptedException, CallError {
							System.out.println("SubscribeToEevent Raised: ");
	...
	public static void startLandMark() {
		try {
			...
			landMark = currentApplication.getAlMemory().subscribeToEvent("LandmarkDetected",
					new EventCallback<ArrayList>() {
						@Override
						public void onEvent(ArrayList o) throws InterruptedException, CallError {
